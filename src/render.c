...
/*   Updated: 2025/06/11 17:37:50 by vpushkar         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void ft_place_basic_tiles(t_game *game, char tile, int x, int y)
{
	...
	if (tile == '1')
	{
		idx = mlx_image_to_window(game->mlx, game->images.wall_img, x * 64, y * 64);
		mlx_resize_image(game->images.wall_img, 64, 64);
		game->images.wall_img->instances[idx].z = 1;
	}
	else if (tile == 'C')
	{
		idx = mlx_image_to_window(game->mlx, game->images.coll_img, x * 64, y * 64);
...
void ft_place_special_tiles(t_game *game, char tile, int x, int y)
{
	...
	if (tile == 'P')
	{
		idx = mlx_image_to_window(game->mlx, game->images.player_img, x * 64, y * 64);
	...
	{
		idx = mlx_image_to_window(game->mlx, game->images.exit_img, x * 64, y * 64);
